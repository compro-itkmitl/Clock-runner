...
void timer(DateTime now); //ประกาศสร้างฟังก์ชั่นเพื่อ print เวลากับวันที่บน LCD
...
// มอเตอร์ A ตั้งเป็น pin output เพื่อปล่อยไฟ
...
// ตั้ง pin buzzer เป็นตัว Output
pinMode(buzzer1, OUTPUT);
pinMode(buzzer2, OUTPUT);

//RTC.adjust(DateTime(__DATE__, __TIME__)); 
//ใช้เพื่อตั้งเวลาใหม่ พอตั้งเสร็จให้ Code ลงบอร์ด แล้วปิดคำสั่งนี้แล้วอัพ code อีกครั้ง

DateTime now = RTC.now();
RTC.setAlarm1Simple(0, 0); // ตั้งเวลาปลุก(ชั่วโมง, นาที)
...
void loop () {
  ...
  if (RTC.checkIfAlarm(1)){
    while(1){ //ทำไปเรื่อยๆจนกว่าจะยอมลุกมาปิดนาฬิกา 
      ...
      digitalWrite(trig, LOW); //กำหนดความยาวคลื่นที่ส่งออกของ Ultrasonic 
